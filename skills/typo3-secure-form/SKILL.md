...
description: Secure TYPO3 frontend forms using the form extension (typo3/cms-form), frontend user registration forms (evoweb/sf-register), and install/configure Google reCAPTCHA via evoweb/recaptcha. Make sure to use this skill whenever the user mentions form security, spam protection, reCAPTCHA, CAPTCHA, sf-register registration, cms-form honeypot, user registration validation, or securing contact/input forms in TYPO3.
---

# TYPO3 Form Security (cms-form, sf-register, and evoweb/recaptcha)

Use this skill to configure, harden, and secure frontend forms in TYPO3 v14 projects, specifically targeting forms generated by the standard form framework (`typo3/cms-form`) and user registration forms (`evoweb/sf-register`), using Google reCAPTCHA (`evoweb/recaptcha`) and honeypot fields.

## When To Use

- Secure contact or feedback forms built with `typo3/cms-form`
- Secure registration, edit-profile, or password-recovery forms built with `evoweb/sf-register`
- Install, configure, or troubleshoot Google reCAPTCHA using the `evoweb/recaptcha` extension
- Add, customize, or disable honeypot spam protection fields
- Whitelist Google reCAPTCHA scripts and frames in Content Security Policies (CSP)
- Configure test or production credentials in `config/system/settings.php`

## Prerequisites

- **TYPO3 v14**
- Extension `typo3/cms-form` installed
- Extension `evoweb/sf-register` (v14+) installed
- Extension `evoweb/recaptcha` (v14+) installed
- Local environment running in DDEV
...
In TYPO3 v14, ensure the main site package theme set (e.g., `packages/theme-skom/Configuration/Sets/skom/config.yaml`) lists the dependencies so their resources, TypoScript, and form configurations are auto-loaded:

```yaml
name: typo3/theme-skom
dependencies:
  - evoweb/recaptcha
  - typo3/form
...
To make the `Recaptcha` field available to forms, register it under your site package's standard form prototype configuration (e.g., `packages/theme-skom/Configuration/Form/SiteForms/config.yaml`):
...
Override or edit the registration template (e.g., `packages/theme-skom/Resources/Private/SfRegister/Partials/Form/Captcha.html`):
...
document.addEventListener('DOMContentLoaded', function() {
    const forms = document.querySelectorAll('#sfrStoreOwnerRegistrationForm, #sfrShopperRegistrationForm');
    forms.forEach(form => {
...
Using Google reCAPTCHA Enterprise requires loading external scripts, frames, and making background API calls. Add the following hosts to your CSP definition file (e.g., `config/sites/main/csp.yaml`):
...
```bash
ddev exec "cd packages/theme-skom && npx playwright test --grep @vrt"
```
